# Welcome, dear programmer, to the magnificent world of hexadecimal to binary conversion!
# This program is designed to take a hexadecimal number, that most enigmatic of numeral systems,
# and transform it into the binary realm, where only 0s and 1s reside.
# Prepare yourself for a journey through loops, conditionals, and variables aplenty!

# Function to convert a single hexadecimal digit to its binary equivalent
def hex_to_bin_digit(hex_digit)
    case hex_digit.upcase
    when '0' then '0000'
    when '1' then '0001'
    when '2' then '0010'
    when '3' then '0011'
    when '4' then '0100'
    when '5' then '0101'
    when '6' then '0110'
    when '7' then '0111'
    when '8' then '1000'
    when '9' then '1001'
    when 'A' then '1010'
    when 'B' then '1011'
    when 'C' then '1100'
    when 'D' then '1101'
    when 'E' then '1110'
    when 'F' then '1111'
    else '????' # In case of an unexpected character
    end
end

# Function to convert an entire hexadecimal string to binary
def hex_to_bin(hex_string)
    # Initialize the binary string as an empty vessel, ready to be filled with binary digits
    binary_string = ""
    
    # Traverse each character in the hexadecimal string
    hex_string.each_char do |char|
        # Convert the current hexadecimal digit to binary and append it to the binary string
        binary_string += hex_to_bin_digit(char)
    end
    
    # Return the fully constructed binary string
    binary_string
end

# Function to check if a string is a valid hexadecimal number
def valid_hex?(hex_string)
    # Regular expression to match a valid hexadecimal number
    !!(hex_string =~ /\A[\da-fA-F]+\z/)
end

# Main program execution starts here
def main
    # The sky is clear, and the user is prompted to enter a hexadecimal number
    puts "Enter a hexadecimal number:"
    hex_input = gets.chomp
    
    # Check if the input is a valid hexadecimal number
    if valid_hex?(hex_input)
        # Convert the valid hexadecimal number to binary
        binary_output = hex_to_bin(hex_input)
        # Display the resulting binary number to the user
        puts "The binary equivalent is: #{binary_output}"
    else
        # Inform the user of their erroneous ways
        puts "Invalid hexadecimal number entered. Please try again."
    end
end

# Call the main function to kick off the program
main

